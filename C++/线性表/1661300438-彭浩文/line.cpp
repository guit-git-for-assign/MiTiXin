...
public:
	...
	void InsertElemeInOrdered(DataType data);
private:
	Node * head; //头结点
};
//初始化单链表
...
void LinkList::CreateLinkList(int n) {
	...
	for (int i = 0; i < n; i++) { //将值一个一个插入单链表中pnew = new Node;
		cout << "NOTE:请按照从小到大的顺序一次输入数据" << endl;
		cout << "请输入第" << i + 1 << "个值: ";
		pnew = new Node;
		cin >> pnew->data;
...
void LinkList::InsertElemAtEnd(DataType data) {
	...
	if (head->next == NULL) { //当头结点为空时，设置newNode为头结点
		head->next = newNode;
...
void LinkList::InsertElemeInOrdered(DataType data) {
	Node * newNode = new Node; //定义一个Node结点指针newNode
	newNode->next = NULL; //定义newNode的数据域和指针域
	newNode->data = data;
	Node * p = head; //定义指针p指向头结点
